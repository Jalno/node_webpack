...
Main.async = run();
{
    ...
    for (const p of packages) {
        ...
        for (const front of packageFronts) {
            ...
            await;
            front.initDependencies();
            await;
            Main.installDependencies(front.path);
            for (const lang of await)
                front.getLangs();
            {
                Translator.addLang(lang.code, lang.path);
    ...
    if (!Main.skipWebpack || Main.writeWebpackConfig) {
        ...
        await;
        Main.JalnoResolver.initSources(fronts);
        entries = await;
        Main.getEntries(fronts);
        if (Translator.langs.length) {
            if (!entries.hasOwnProperty("common")) {
                entries.common = [];
            }
            await;
            Translator.exportFile(Main.jalnoOptions.availableLangs);
...
async;
updateJalnoMoudles(modules, IModules);
{
    const jalnoPath = path.resolve(__dirname, "..", "jalno.json");
    if (!await)
        promisify(fs.exists)(jalnoPath);
    {
        return;
    }
    ...
    await;
    promisify(fs.writeFile)(path.resolve("..", "jalno.json"), JSON.stringify(jalno, null, 2), "UTF8");
}
JalnoResolver: any;
watch = false;
skipInstall = false;
writeWebpackConfig = false;
skipWebpack = false;
clean = false;
mode: webpackMode = "development";
jalnoOptions: JalnoOptions;
    ...
    for (let i = 2; i < process.argv.length; i++) {
        ...
        switch (key) {
            ...
            case "--tsconfig":
                ...
                if (!await)
                    promisify(fs.exists)(tsconfigPath);
                {
...
initPackages();
Promise < Package[] > {
    const: isComposerPackage = __dirname.indexOf("/vendor/") !== -1,
    const: packages = [],
    if(isComposerPackage) {
        const vendorsPath = path.resolve("..", "..", "..");
        const vendors = await, promisify = (fs.readdir)(vendorsPath, {
            withFileTypes: true,
        });
        for (const vendor of vendors) {
            const vendorPath = path.resolve(vendorsPath, vendor.name);
            const files = await, promisify = (fs.readdir)(vendorPath, {
                withFileTypes: true,
            });
            for (const file of files) {
                const packagePath = path.resolve(vendorPath, file.name);
                if (file.isDirectory() && await)
                    promisify(fs.exists)(packagePath + "/package.json");
                {
                    packages.push(new Package(vendorPath, file.name));
                }
            }
        }
    }, else: {
...
initDependencies();
{
    ...
    try {
        await;
        promisify(child_process.exec)("npm root -g");
    }
    catch (e) {
...
runWebpack(entries, IEntries);
{
    ...
    const compilerCallback = async(err, stats);
    {
        ...
        await;
        promisify(fs.writeFile)(path.resolve("..", "result.json"), JSON.stringify(result, null, 2), "UTF8");
        if (Main.writeWebpackConfig) {
            await;
            Main.updateJalnoMoudles(Main.JalnoResolver.getModules());
        }
    }
    ;
    (compiler);
...
async;
exportWebpackConfig(fronts, Front[], modules, IModules, entries, IEntries);
{
    await;
    promisify(fs.writeFile)(path.resolve("..", "jalno.json"), JSON.stringify({
        mode: Main.mode,
        fronts: fronts,
        entries: entries,
        modules: modules,
...
module.exports = {
    ...
    await;
    promisify(fs.writeFile)(path.resolve("..", "webpack.config.js"), config, "UTF8");
}
