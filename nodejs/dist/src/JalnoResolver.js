...
JalnoResolver.async = initSources(fronts, Front[]);
{
...
setModules(modules, IModules);
{
    JalnoResolver.modules = modules;
}
setFronts(fronts, Front[]);
{
    JalnoResolver.fronts = fronts;
}
isCommonModule(module, any);
{
...
modules: IModules = {};
async;
lookingForPackage(name, string, basepath, string, packagePath, string);
Promise < Module > {
    const: realpath = packagePath,
    const: realPackageManager = await, JalnoResolver: .getPackage(packagePath),
    let: front, Front: ,
    for( = item, of = JalnoResolver.fronts) {
...
if (JalnoResolver.modules[name] !== undefined) {
    ...
    if (packageManager) {
        ...
        JalnoResolver.getPackage(resolve(front.path, "package.json"));
...
if (realPackageManager && realPackageManager.hasOwnProperty("dependencies") && realPackageManager.dependencies[name] !== undefined) {
    ...
    let dir = realpath;
    while (dir !== "/") {
        newPackageManager = await;
        JalnoResolver.getPackage(resolve(dir, name, "package.json"));
...
constructor(private, source, private, target);
{ }
async;
apply(resolver, any);
{
            ...
            if (lastSplash >= 0) {
                ...
                if (formats.indexOf(ext) === -1) {
                    for (const format of formats) {
                        if (await)
                            exists(newModule.request + "." + format);
                    }
                }
            }
        } });
